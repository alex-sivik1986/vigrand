...
function wc_products_rss_feed() {
	...
	if ( is_post_type_archive( 'product' ) || is_singular( 'product' ) ) {
		...
		echo '<link rel="alternate" type="application/rss+xml"  title="' . esc_html__( 'New products', 'doors' ) . '" href="' . esc_attr( $feed ) . '" />';
		...
		echo '<link rel="alternate" type="application/rss+xml"  title="' . sprintf(__( 'New products added to %s', 'doors' ), urlencode($term->name)) . '" href="' . esc_attr( $feed ) . '" />';
		...
		echo '<link rel="alternate" type="application/rss+xml"  title="' . sprintf(__( 'New products tagged %s', 'doors' ), urlencode($term->name)) . '" href="' . esc_attr( $feed ) . '" />';
...
if ( ! function_exists( 'woocommerce_demo_store' ) ) {
	...
	function woocommerce_demo_store() {
		...
		if ( empty( $notice ) )
			$notice = esc_html__( 'This is a demo store for testing purposes &mdash; no orders shall be fulfilled.', 'doors' );
...
if ( ! function_exists( 'woocommerce_page_title' ) ) {
	...
	function woocommerce_page_title( $echo = true ) {
		...
		if ( is_search() ) {
			$page_title = sprintf( esc_html__( 'Search Results: &ldquo;%s&rdquo;', 'doors' ), get_search_query() );

			if ( get_query_var( 'paged' ) )
				$page_title .= sprintf( esc_html__( '&nbsp;&ndash; Page %s', 'doors' ), get_query_var( 'paged' ) );
...
if ( ! function_exists( 'woocommerce_catalog_ordering' ) ) {
	...
	function woocommerce_catalog_ordering() {
		...
		$catalog_orderby_options = apply_filters( 'woocommerce_catalog_orderby', array(
			'menu_order' => esc_html__( 'Default sorting', 'doors' ),
			'popularity' => esc_html__( 'Sort by popularity', 'doors' ),
			'rating'     => esc_html__( 'Sort by average rating', 'doors' ),
			'date'       => esc_html__( 'Sort by newness', 'doors' ),
			'price'      => esc_html__( 'Sort by price: low to high', 'doors' ),
			'price-desc' => esc_html__( 'Sort by price: high to low', 'doors' )
		) );
	...
	function woocommerce_default_product_tabs( $tabs = array() ) {
		...
		if ( $post->post_content ) {
			$tabs['description'] = array(
				'title'    => esc_html__( 'Description', 'doors' ),
				'priority' => 10,
		...
		if ( $product && ( $product->has_attributes() || ( $product->enable_dimensions_display() && ( $product->has_dimensions() || $product->has_weight() ) ) ) ) {
			$tabs['additional_information'] = array(
				'title'    => esc_html__( 'Additional Information', 'doors' ),
				'priority' => 20,
		...
		if ( comments_open() ) {
			$tabs['reviews'] = array(
				'title'    => sprintf( esc_html__( 'Reviews (%d)', 'doors' ), get_comments_number( $post->ID ) ),
				'priority' => 30,
...
if ( ! function_exists( 'woocommerce_related_products' ) ) {
	...
	function woocommerce_related_products( $args = array(), $columns = false, $orderby = false ) {
		if ( ! is_array( $args ) ) {
			_deprecated_argument( __FUNCTION__, '2.1', esc_html__( 'Use $args argument as an array instead. Deprecated argument will be removed in WC 2.2.', 'doors' ) );
...
if ( ! function_exists( 'woocommerce_order_again_button' ) ) {
	...
	function woocommerce_order_again_button( $order ) {
		...
		?>
		<p class="order-again">
			<a href="<?php echo wp_nonce_url( add_query_arg( 'order_again', $order->id ) , 'woocommerce-order_again' ); ?>" class="button"><?php esc_html_e( 'Order Again', 'doors' ); ?></a>
		</p>
		<?php
	}
}
...
if ( ! function_exists( 'woocommerce_form_field' ) ) {
	...
	function woocommerce_form_field( $key, $args, $value = null ) {
		...
		if ( $args['required'] ) {
			...
			$required = ' <abbr class="required" title="' . esc_attr__( 'required', 'doors'  ) . '">*</abbr>';
		} else {
			$required = '';
		}
		...
		case "country" :
			...
			if ( sizeof( $countries ) == 1 ) {
				...
				$field = '<p class="form-row ' . esc_attr( implode( ' ', $args['class'] ) ) .'" id="' . esc_attr( $args['id'] ) . '_field">'
						...
						. '<option value="">'.__( 'Select a country&hellip;', 'doors' ) .'</option>';
				...
				$field .= '<noscript><input type="submit" name="woocommerce_checkout_update_totals" value="' . esc_html__( 'Update country', 'doors' ) . '" /></noscript>';
		...
		case "state" :
			...
			if ( is_array( $states ) && empty( $states ) ) {
				...
				$field .= '<select name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" class="state_select" ' . implode( ' ', $custom_attributes ) . ' placeholder="' . esc_attr( $args['placeholder'] ) . '">
					<option value="">'.__( 'Select a state&hellip;', 'doors' ) .'</option>';
...
if ( ! function_exists( 'get_product_search_form' ) ) {
	...
	function get_product_search_form( $echo = true  ) {
		...
		$form = '<form method="get" id="searchform" action="' . esc_url( home_url( '/'  ) ) . '">
			<div>
				<label class="screen-reader-text" for="s">' . esc_html__( 'Search for:', 'doors' ) . '</label>
				<input type="text" value="' . get_search_query() . '" name="s" id="s" placeholder="' . esc_html__( 'Search for products', 'doors' ) . '" />
				<input type="submit" id="searchsubmit" value="'. esc_attr__( 'Search', 'doors' ) .'" />
